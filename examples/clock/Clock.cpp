...
{
  char buf[64];
  sprintf(buf, "%s, %s %d, %02d", toWeekString(getWeek()), toMonthString(getMonth()), getDay(), getYear());
  sbuf = buf;
  return sbuf.c_str();
}
...
{
  char buf[32];
  sprintf(buf, "%02d%s%02d", getHour(), ((getSecond() % 2) == 0) ? ":" : " ", getMinute());
  sbuf = buf;
  return sbuf.c_str();
}
...
{
  char buf[128];
  sprintf(buf, "%s, %s %d, %02d, %02d:%02d:%02d", toWeekString(getWeek()), toMonthString(getMonth()), getDay(), getYear(), getHour(), getMinute(), getSecond());
  sbuf = buf;
  return sbuf.c_str();
}
